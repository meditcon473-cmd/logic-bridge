...
import { ZhiyinState, EssayEngineData } from "./state";
import { METAPHOR_PROMPT, PHENOMENON_PROMPT, DIALECTICAL_PROMPT, SYNTHESIS_PROMPT } from "./config/prompts";
import { RAG_REALMS, getRealmsByNode } from "./knowledge/rag_realms";

// ============================================
// 1. 节点函数定义 (Dummy Nodes + Prompt 装配)
// ============================================

async function snifferNode(state: ZhiyinState): Promise<any> {
  console.log(`[SnifferNode] 收到原始题目: "${state.raw_prompt}"`);
  console.log(`[SnifferNode] 当前节点: ${state.current_node}`);

  let engineData: EssayEngineData;
...
async function clarificationNode(state: ZhiyinState): Promise<any> {
  console.log(`[ClarificationNode] 进入澄清节点`);
  console.log(`[ClarificationNode] 需要澄清: ${(state.engine_data as any).clarification_needed}`);
 ...
 * 装配 METAPHOR_PROMPT
 */
async function metaphorEngineNode(state: ZhiyinState): Promise<any> {
  console.log(`[MetaphorEngineNode] 进入隐喻引擎`);
  
  // 装配 Prompt（模板 + 当前状态变量）
  const assembledPrompt = `${METAPHOR_PROMPT}\n\n当前题目: ${state.raw_prompt}`;
  console.log(`[MetaphorEngineNode] 装配后的 Prompt:\n${assembledPrompt}\n`);
  
  // 演示 RAG 检索
  const jiang1Realms = getRealmsByNode("jiang_1");
  console.log(`[MetaphorEngineNode] RAG 检索 - jiang_1 知识库: ${jiang1Realms.length} 条`);
  
  const mockData: Extract<EssayEngineData, { type: "METAPHOR" }> = {
    type: "METAPHOR",
 ...
 */
async function phenomenonEngineNode(state: ZhiyinState): Promise<any> {
  console.log(`[PhenomenonEngineNode] 进入现象引擎`);
  
  // 装配 Prompt
  const assembledPrompt = `${PHENOMENON_PROMPT}\n\n当前题目: ${state.raw_prompt}`;
  console.log(`[PhenomenonEngineNode] 装配后的 Prompt:\n${assembledPrompt}\n`);
  
  // 演示 RAG 检索
  const jiang2Realms = getRealmsByNode("jiang_2");
  console.log(`[PhenomenonEngineNode] RAG 检索 - jiang_2 知识库: ${jiang2Realms.length} 条`);
  
  const mockData: Extract<EssayEngineData, { type: "PHENOMENON" }> = {
    type: "PHENOMENON",
    subject: "AI时代",
 ...
 */
async function dialecticalEngineNode(state: ZhiyinState): Promise<any> {
  console.log(`[DialecticalEngineNode] 进入思辨引擎`);
  
  // 装配 Prompt
  const assembledPrompt = `${DIALECTICAL_PROMPT}\n\n当前题目: ${state.raw_prompt}`;
  console.log(`[DialecticalEngineNode] 装配后的 Prompt:\n${assembledPrompt}\n`);
  
  // 演示 RAG 检索
  const jiang3Realms = getRealmsByNode("jiang_3");
  console.log(`[DialecticalEngineNode] RAG 检索 - jiang_3 知识库: ${jiang3Realms.length} 条`);
  
  const mockData: Extract<EssayEngineData, { type: "DIALECTICAL" }> = {
    type: "DIALECTICAL",
    concept_A: "科技发展",
    concept_B: "人文关怀",
 ...
 * 装配 SYNTHESIS_PROMPT
 */
async function synthesisNode(state: ZhiyinState): Promise<any> {
  console.log(`[SynthesisNode] 进入数据汇流器`);
  
  // 装配 Prompt
  const assembledPrompt = `${SYNTHESIS_PROMPT}\n\n引擎数据: ${JSON.stringify(state.engine_data)}\n原始题目: ${state.raw_prompt}`;
...
// 六段流水线
async function fengtouNode(state: ZhiyinState): Promise<any> {
  console.log(`[fengtouNode] 凤头 - 开头段落`);
  return { current_node: "fengtouNode" };
}

async function jiang1Node(state: ZhiyinState): Promise<any> {
  console.log(`[jiang1Node] 将1 - 第一个分论点`);
  // 使用 RAG 知识库
  const realms = getRealmsByNode("jiang_1");
...
async function jiang2Node(state: ZhiyinState): Promise<any> {
  console.log(`[jiang2Node] 将2 - 第二个分论点`);
  const realms = getRealmsByNode("jiang_2");
...
async function zhuanNode(state: ZhiyinState): Promise<any> {
  console.log(`[zhuanNode] 转 - 转折段落`);
  return { current_node: "zhuanNode" };
}

async function jiang3Node(state: ZhiyinState): Promise<any> {
  console.log(`[jiang3Node] 将3 - 第三个分论点`);
  const realms = getRealmsByNode("jiang_3");
...
async function baoweiNode(state: ZhiyinState): Promise<any> {
...
function routeFromSniffer(state: ZhiyinState): string {
...
const ZhiyinStateSchema = z.object({
  raw_prompt: z.string(),
  ...
  error_logs: z.array(z.string()),
});

export function buildZhiyinGraph() {
  const workflow = new StateGraph(ZhiyinStateSchema);
...
export async function runZhiyin(prompt: string) {
  const graph = buildZhiyinGraph();

  const initialState: ZhiyinState = {
    raw_prompt: prompt,
    messages: [],
